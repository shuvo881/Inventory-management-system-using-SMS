...
<?php
  ...
  if(isset($_POST['regBtn'])){
    ...
    if($_POST['password'] == $_POST['cpassword'] && $_POST['cpassword'] != NULL){
      ...
      $sql = "SELECT scrtPassword FROM secretpassword";
      $result = mysqli_query($msqlConn,$sql);
      $row = mysqli_fetch_row($result);

      if($msqlConn && $row[0] == $_POST['spassword'] ) {
        $fName = $_POST['fname'];
        $numbr = $_POST['number'];
        $lName = $_POST['lname'];
        $eml = $_POST['email'];
        $pass = $_POST['password'];
        $_sql = "INSERT INTO admininfo VALUES('$fName','$lName','$eml','$numbr','$pass')";
    ...
    <div class="Registation-now">
      <form class="" action="#" method="post">
        ...
        <div class="input-style">
          ...
          <label for="a">Secret Password</label>
          <input type="password" name="spassword" placeholder="secret password" id="g">
        </div>
